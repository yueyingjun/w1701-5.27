/*
  函数 colorAnimate (obj,attr,val,dur,fun,callback)
  obj   要处理的对象
...
/*
   ...
   obj   要动画的对象
   attrobj   要动画的属性对象 是一个数{width:xxxx,height:xxx,left:xxxx,top:xxxx,opacity:xxx}
                                        width 和其他都不写单位
   dur   持续时间
   fun   动画方式  默认Tween.Linear 匀速  (一般不写)
   callback 变化完成之后要处理的内容  回调函数
*/

//动画算法
/* fun  动画方式  Tween.Linear
		  Tween.Linear：无缓动效果(匀速运动)； 
			Tween.Quad：二次方的缓动；
			Tween.Cubic：三次方的缓动
			Tween.Quartic：四次方的缓动；
			Tween.Quintic：五次方的缓动；
			Tween.Sinusoidal：正弦曲线的缓动；
			Tween.Exponential：指数曲线的缓动；
			Tween.Circular：圆形曲线的缓动；
			Tween.Elastic：指数衰减的正弦曲线缓动；
			Tween.Back：超过范围的三次方缓动）；
			Tween.Bounce：指数衰减的反弹缓动。
